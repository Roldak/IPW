...
trait FunctionEvaluator
  ...
  protected[utils] def e(expr: Expr)(implicit rctx: RC, gctx: GC): Expr = expr match {
    case v: Variable =>
      rctx.mappings.get(v.toVal) match {
        case Some(x) => 
          println("subst " + v + " for " + x)
          x
        case None => v
      }

    case Application(caller, args) =>
      Application(e(caller), args map e)
    ...
    case FunctionInvocation(id, tps, args) =>
      ...
      println("FUNC: " + tfd.fullBody + " ARGS: " + evArgs + "FRAME: " + rctx)

      if (gctx.stepsLeft < 0) {
        return FunctionInvocation(id, tps, evArgs)
      }
      ...
      val tmp = e(tfd.fullBody)(frame, gctx)
      println("AFTER SUBST: " + tmp + frame)
      tmp
...
object FunctionEvaluator {
  def apply(p: InoxProgram, opts: Options): FunctionEvaluator { val program: p.type } = {
    new {
      val program: p.type = p
      val options = opts
    } with FunctionEvaluator with HasDefaultGlobalContext with HasDefaultRecContext {
